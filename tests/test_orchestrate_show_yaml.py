...
class TestRunShowYaml:
    def test_no_pending_returns_zero(self, capsys):
        rc = _run_show_yaml(_args(_eco(), None), _FakeDB([]))
        assert rc == 0
...
class TestRunOrchestrateShowYamlDispatch:
    def test_show_yaml_flag_routes_to_preview(self, tmp_path: Path, capsys):
