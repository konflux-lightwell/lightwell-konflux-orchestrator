...
def add_common_orchestrate_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--release-data-repo",
        default=None,
        help="Path to a konflux-release-data checkout for --show-yaml "
...
def run_orchestrate(args: argparse.Namespace, empty_db_warning: str) -> int:
    ...
    with ImportDatabase(args.db) as db:
        ...
        if getattr(args, "show_yaml", False):
            return _run_show_yaml(args, db)
...
def _run_show_yaml(args: argparse.Namespace, db: ImportDatabase) -> int:
